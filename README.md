...
- phononDOS.dat calculates the constant volume specific heat Cv_DOS and Debye temperature. If the bulk modulus or density in parameter.txt are <= 0, it calculates the average sound speed from Cv_DOS and the thermal conductivity.
- The temperature dependence is calculated using the Cezairliyan treatment in "Thermal Conductivity of the Elements", where the code assumes that Tm = Debye temperature and km is the thermal conductivity at the Debye temperature.
