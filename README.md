...
- H. Sato et al., J. Phase Equilib. Diffus. 45, 397-415 (2024).: [https://doi.org/10.1007/s11669-024-01086-y](https://doi.org/10.1007/s11669-024-01086-y) (LBT-TETRA)
...
## Treatment of lattice thermal conductivity ($$\kappa_{ph}$$) in Seebeck_analysis.f90
...
- This code uses elasticity tensor values (especially the bulk modulus and shear modulus or Poisson's ratio) to calculate the speed of sound, and also calculates the Debye temperature, Grüneisen constant, and lattice thermal conductivity.
- Calculating thermal conductivity requires third-order calculations, resulting in very high computational costs. On the other hand, methods using empirical parameters based on elasticity tensors (especially the bulk modulus and shear modulus or Poisson's ratio) are computationally inexpensive and are expected to be useful for materials screening.
- This code also includes a calculation method for elasticity tensors (stored in Elastic_consntants_calculation). For QE, we recommend using thermo_pw. If you insist on full-potential calculations, we recommend using ElaStic with exciting (Exciting_ElaStic_v1.1). Of course, ElaStic can also be used with QE (QE_Elastic2020).
- "Bulk_modulus_calculation" and "Shear_modulus_calculation" are original codes created almost from scratch, but they only work well with conventional cells or Cartesian coordinates. Ultimately, limiting the calculation target to bulk modulus or shear modulus does not improve calculation speed for systems with primitive cells. There may be advantages for systems with reduced symmetry due to elemental substitution or defects, requiring calculations with conventional cells or Cartesian coordinates.
- In one paper [17], the constant A in Slack's formula is calculated by taking into account dB/dP and dG/dP to obtain the Grüneisen constant, which matches the thermal conductivity. Predicting the constant A and the Grüneisen constant in Slack's formula from material properties with lower computational cost and fitting them to the thermal conductivity is expected to enable rapid material discovery for thermal conductivity.
- This time, I have made significant improvements to the elasticity tensor calculation, and I hope to see progress not only in the fields of materials mechanics, but also in heat conduction and thermoelectricity.
...
2. arXiv version
```
_A preprint describing QM2Thermo will be uploaded to arXiv when time permits._
```

## License
This project is licensed under the MIT License. Please check the LICENSE file for more information.

QM2Thermo is a successor to the previously published LBT-TETRA framework. While LBT-TETRA laid the foundation for Seebeck coefficients estimation, QM2Thermo introduces significant improvements, including temperature-dependent modeling via Slack's and Slater's formulations, enhanced support for substituted systems, and a restructured codebase with over 50% original implementation.

In addition to the algorithmic advancements, QM2Thermo transitions the codebase from Fortran 77 to Fortran 90, enabling modular design, improved numerical stability, and better compatibility with modern computational environments. This modernization facilitates easier integration with high-throughput workflows and enhances maintainability for future development.
...
| **DOS-based (phonon)**       | $$\tau^{-1}(\varepsilon, T) = \frac{1}{\tau_0} \cdot \int D_{\text{ph}}(\omega) \cdot \frac{1 + n(\omega,T)}{\omega} \, d\omega$$ | Indirect via phonons        | Strong via $$n(\omega)$$   | Full phonon spectrum  |
| **DOS-based ($$a^{2}F(\omega)$$)**       | $$\tau^{-1}(\varepsilon, T) = \frac{1}{\tau_0} \cdot 2\pi \int_0^{\infty} \alpha^2F(\omega) \left[1 + 2n(\omega) + f(\varepsilon - \omega) - f(\varepsilon + \omega)\right] d\omega$$ | Indirect via phonons        | Strong via $$n(\omega)$$   | Full phonon spectrum |
...
> Notes:
> $$a^{2}F(\omega)$$ is preferred over DOS-based (phonon).
> - $$\( g(\varepsilon) \)$$: electronic density of states  
...
| **DOS-based (phonon)**       | $$\tau^{-1} = \tau_0^{-1} \times \int D_{\text{ph}}(\omega) \cdot \frac{1 + n(\omega,T)}{\omega} d\omega$$ | Indirect via phonons        | Strong via $$\( n(\omega) \)$$ | Full phonon spectrum      |
