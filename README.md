...
| `plot_AB.gpl`   | `Seebeck_analysis.dat`          | A(T) and B(T) | Plot the numerator A(T) and denominator B(T) as shown in Figure 12 in the paper. To specify the bands as in Figure 12, use select_band_range.txt to perform calculations from group_velocity.f90 to output AKK.DATA with the specified bands. Note that the paper uses tau0 = 1.0 for the calculations. A(T) is the integral with respect to the energy E at A(E,T), and B(T) is the integral with respect to the energy E at B(E,T). |
