...
- One paper [17] calculates the Grüneisen constant in Slack's formula, taking into account dB/dP and dG/dP, so that it matches the thermal conductivity. It is predicted that predicting the constant A and the Grüneisen constant in Slack's formula by fitting them from physical property values with lower computational costs will be important for rapid material discovery.
