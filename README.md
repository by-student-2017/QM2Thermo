...
- One paper [17] calculates the Grüneisen constant in Slack's formula, taking into account dB/dP and dG/dP, so that the constant A in Slack's formula matches the thermal conductivity. Fitting the Slack formula to physical property values with lower computational costs is expected to be important for rapid material discovery. I look forward to further progress in the future.
