...
!   Ñ•ÑüÑü Step 0: Load Electron-Phonon Coupling Data
!   Ñ†   Ñ•ÑüÑü Reads phonon coupling constants from 'lambda'.
!   Ñ†   Ñ•ÑüÑü Initializes arrays for broadening, lambdaArray, dosEf, and omega_ln.
!   Ñ†   Ñ§ÑüÑü Uses `CALL ReadLambdaData()` for data processing.
!   Ñ†
!   Ñ•ÑüÑü Step 1: Load Band Velocity Data
!   Ñ†   Ñ•ÑüÑü Reads energy mesh, density of states (DOS), squared group velocity,
!   Ñ†   Ñ†   and band energy levels from 'AKK.DATA'.
!   Ñ†   Ñ•ÑüÑü Applies energy shift if necessary.
!   Ñ†   Ñ§ÑüÑü Computes valence electron concentration (VEC) for given energy levels.
!   Ñ†
!   Ñ•ÑüÑü Step 2: Load Chemical Potentials
!   Ñ†   Ñ•ÑüÑü Reads temperature-dependent chemical potentials (AMU) from 'apot.data'.
!   Ñ†   Ñ§ÑüÑü Associates each temperature (TEM) with its corresponding chemical potential.
!   Ñ†
!   Ñ•ÑüÑü Step 3: Initialize Output File
!   Ñ†   Ñ•ÑüÑü Prepares 'Seebeck_analysis.dat' file for results storage.
!   Ñ†   Ñ•ÑüÑü Includes headers with energy offsets, VEC, and Seebeck coefficient columns.
!   Ñ†
!   Ñ•ÑüÑü Step 4: Read Phonon DOS or a2F.dos Data
!   Ñ†   Ñ•ÑüÑü Calls `read_phonon_dos()` if phononDOS is enabled.
!   Ñ†   Ñ•ÑüÑü Calls `read_a2F_dos()` if a2F.dos is enabled.
!   Ñ†
!   Ñ•ÑüÑü Step 5: Compute Seebeck Coefficient
!   Ñ†   Ñ•ÑüÑü Loops through temperatures to calculate Seebeck coefficient.
!   Ñ†   Ñ•ÑüÑü Computes Fermi-Dirac distribution and its derivative at each energy level.
!   Ñ†   Ñ•ÑüÑü Multiplies by group velocity squared and energy-dependent scattering rate.
!   Ñ†   Ñ•ÑüÑü Accumulates numerator and denominator using integration techniques:
!   Ñ†   Ñ†   Ñ§ÑüÑü Options for Riemann sum, trapezoidal rule, or Simpson's rule.
!   Ñ†   Ñ§ÑüÑü Outputs averaged energy deviation <E - mu>, Seebeck coefficient, and Nc.
!   Ñ†
!   Ñ§ÑüÑü Finalize:
!       Ñ•ÑüÑü Closes output files.
!       Ñ•ÑüÑü Deallocates arrays for phonon and scattering data if used.
!       Ñ§ÑüÑü Ensures proper cleanup for each flag (use_phonon, use_phononDOS, use_a2Fdos).
!
! Input Files:
!   Ñ•ÑüÑü lambda          : Electron-phonon coupling constants and DOS data.
!   Ñ•ÑüÑü AKK.DATA        : Energy mesh, squared velocity, squared velocity * DOS, DOS, Cumulative DOS.
!   Ñ•ÑüÑü apot.data       : Temperature (TEM) and chemical potential (AMU) values.
!   Ñ•ÑüÑü phononDOS.dat   : Phonon DOS data (optional).
!   Ñ•ÑüÑü a2F.dos         : Eliashberg function data (optional).
!
! Output Files:
!   Ñ•ÑüÑü Seebeck_analysis.dat : Results file containing computed Seebeck coefficients.
!-----------------------------------------------------------------------
...
!   get_tau(E1, T, I)
!   Ñ•ÑüÑü Purpose:
!   Ñ†   Compute the relaxation time tau(E, T) based on multiple scattering 
!   Ñ†   mechanisms combined using Matthiessen's rule.
!   Ñ†
!   Ñ•ÑüÑü Scattering Mechanisms:
!   Ñ†   1. Phonon scattering (tau_ph)      : Dependent on |E - mu|, T, and Lambda.
!   Ñ†   2. Impurity scattering (tau_imp)   : User-defined |E - mu|^n exponent model.
!   Ñ†   3. DOS-based scattering (tau_dos)  : Dependent on DOS(E) * T.
!   Ñ†   4. Phonon DOS-based scattering (tau_phdos): Advanced scattering using phonon data.
!   Ñ†
!   Ñ•ÑüÑü Subroutines and Functions Called:
!   Ñ†   Ñ•ÑüÑü GetLambda(E1, T, I)
!   Ñ†   Ñ†   Ñ•ÑüÑü Purpose: Interpolates the electron-phonon coupling constant Lambda.
!   Ñ†   Ñ†   Ñ•ÑüÑü Inputs:
!   Ñ†   Ñ†   Ñ†   - E1: Energy offset from chemical potential.
!   Ñ†   Ñ†   Ñ†   - T: Temperature.
!   Ñ†   Ñ†   Ñ†   - I: Energy index.
!   Ñ†   Ñ†   Ñ•ÑüÑü Output:
!   Ñ†   Ñ†   Ñ†   - Lambda: Electron-phonon coupling constant.
!   Ñ†   Ñ†   Ñ§ÑüÑü Dependencies:
!   Ñ†   Ñ†       - InterpolateLambda(E1, omega_ln, lambdaArray): Performs interpolation.
!   Ñ†   Ñ†       - ReadLambdaData(): Loads Lambda and phonon coupling data from 'lambda'.
!   Ñ†   Ñ†
!   Ñ†   Ñ•ÑüÑü tau_a2Fdos_ET(E1, T)
!   Ñ†   Ñ†   Ñ•ÑüÑü Purpose: Computes relaxation time from Eliashberg function (a2F.dos data).
!   Ñ†   Ñ†   Ñ•ÑüÑü Inputs:
!   Ñ†   Ñ†   Ñ†   - E1: Energy offset from chemical potential.
!   Ñ†   Ñ†   Ñ†   - T: Temperature.
!   Ñ†   Ñ†   Ñ§ÑüÑü Output:
!   Ñ†   Ñ†       - Relaxation time based on phonon density and Bose-Einstein distribution.
!   Ñ†   Ñ†       - Dependencies: read_a2F_dos(), kernel_selection(), integration over a2F_total.
!   Ñ†   Ñ†
!   Ñ†   Ñ•ÑüÑü tau_phdos_ET(E1, T)
!   Ñ†   Ñ†   Ñ•ÑüÑü Purpose: Computes relaxation time based on phonon DOS and n_B (Bose-Einstein).
!   Ñ†   Ñ†   Ñ•ÑüÑü Inputs:
!   Ñ†   Ñ†   Ñ†   - E1: Energy offset from chemical potential.
!   Ñ†   Ñ†   Ñ†   - T: Temperature.
!   Ñ†   Ñ†   Ñ§ÑüÑü Output:
!   Ñ†   Ñ†       - Relaxation time derived from phonon density of states (phononDOS.dat).
!   Ñ†   Ñ†       - Dependencies: read_phonon_dos(), kernel_selection(), phonon integrals, (GetLambda()).
!   Ñ†   Ñ†
!   Ñ†   Ñ•ÑüÑü read_a2F_dos()
!   Ñ†   Ñ†   Ñ•ÑüÑü Purpose: Reads phonon density of states from a2F.dos.
!   Ñ†   Ñ†   Ñ§ÑüÑü Output: Frequency-dependent a2F data.
!   Ñ†   Ñ†
!   Ñ†   Ñ•ÑüÑü read_phonon_dos()
!   Ñ†   Ñ†   Ñ•ÑüÑü Purpose: Reads phonon density of states from phononDOS.dat.
!   Ñ†   Ñ†   Ñ§ÑüÑü Output: Frequency-dependent phonon DOS data.
!   Ñ†   Ñ†
!   Ñ†   Ñ•ÑüÑü kernel_selection(E, omega)
!   Ñ†   Ñ†   Ñ•ÑüÑü Purpose: Determines scattering kernel weights based on energy and phonon frequencies.
!   Ñ†   Ñ†   Ñ•ÑüÑü Inputs:
!   Ñ†   Ñ†   Ñ†   - E: Energy.
!   Ñ†   Ñ†   Ñ†   - omega: Phonon frequency.
!   Ñ†   Ñ†   Ñ§ÑüÑü Output: Logical weight applied to scattering integrals.
!   Ñ†   Ñ†
!   Ñ†   Ñ§ÑüÑü DOS(I)
!   Ñ†       Ñ•ÑüÑü Purpose: Provides the density of states at a given energy index.
!   Ñ†       Ñ§ÑüÑü Dependencies: read_constants() (for grid setup) and external DOS data.
!   Ñ†
!   Ñ•ÑüÑü Integration Techniques Used:
!   Ñ†   - MatthiessenÅfs rule combines scattering rates (1/tau) for active mechanisms.
!   Ñ†   - Fallback to tau0 if no active scattering mechanisms are enabled.
!   Ñ†
! Inputs to get_tau:
...
!       phonon energy É÷ is less than electron energy offset |E|
!
! Usage:
!   - Used by tau_phdos_ET(E,T) for filtering É÷ contributions
...
200 CONTINUE
  ...
  !          : based on MatthiessenÅfs rule combining multiple scattering mechanisms:
  ...
  SUBROUTINE read_phonon_dos()
    ...
    norm_factor = 3.0D0 * N_atom / (sum + 1.0D-12)   ! for exampleÅF3N = 3 * N_atom
